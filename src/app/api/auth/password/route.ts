...
export async function POST(request: NextRequest) {
  ...
  if (typeof password !== "string" || password.length < 8) {
    return NextResponse.json({ message: "La contraseña debe tener al menos ocho caracteres." }, { status: 400 });
  }
  ...
  try {
    ...
    if (error) return NextResponse.json({ message: "El enlace ya no es válido. Solicitá uno nuevo desde el acceso." }, { status: 401 });
    return response;
  } catch {
